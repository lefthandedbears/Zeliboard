 ...
 */
class Suggest(private val mDictionaryFacilitator: DictionaryFacilitator, private val context: Context? = null) {
    ...
    /**
     * Zeliboard glide-typing fallback: use GlideTypingClassifier when no native library decoded
     * the gesture. Seeds the classifier with typing-mode candidates for the gesture's first letter.
     * Returns null if the gesture path is too short or the keyboard has no letter keys.
     */
    private fun getGlideTypingSuggestions(
        ...
        // Detect the first letter touched and fetch typing-mode candidates starting with it
        val firstX = inputPointers.xCoordinates[0]
        ...
        if (typingCandidates.isEmpty()) return null
        ...
        context?.let { ctx ->
            rankedCandidates = GesturePersonalization.getInstance(ctx).applyBoosts(rankedCandidates)
        }

        val results = SuggestionResults(
            rankedCandidates.size,
            typingCandidates.mIsBeginningOfSentence,
            false
        )
        results.addAll(rankedCandidates)
        return results
    }
