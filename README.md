...
**Zeliboard** is a personal, privacy-first Android keyboard forked from [HeliBoard](https://github.com/Helium314/HeliBoard).
It extends HeliBoard with an on-device swipe/glide typing engine — no network calls, no cloud processing, all data stays on your device.

## What makes it Zeliboard?

| Feature | Details |
|---|---|
| **Swipe / glide typing** | Ported from FlorisBoard's `StatisticalGlideTypingClassifier` (Apache 2.0). Pure Kotlin, zero native library required. |
| **Personalised ranking** | Words you frequently swipe-confirm are ranked higher over time. Model lives entirely in SharedPreferences on your device. |
| **Total privacy** | No internet permission. Dictionary, gesture model, and personalisation all run on-device. |
| **Always improving** | Each accepted swipe word feeds back into the scoring model automatically. |
...
The APK will be at `app/build/outputs/apk/debug/Zeliboard_3.9-zeli-debug.apk`.

---

_Upstream: HeliBoard — see original README below._
